...
set(gca,'Visible','off')
    ...
    function senpai_prune_done(src,event)
        save(fullfile(save_path,'markers.mat'),'markers')
...
function [J, region] = regionGrowing(I, seed, ~, radius)
% Local fallback for the marker placement used by senpai_prune.
if nargin<4
    radius=5;
end
seed=round(seed);
...
seed=max(seed,[1 1 1]);
seed=min(seed,sz);
[xg,yg,zg]=ndgrid(1:sz(1),1:sz(2),1:sz(3));
region=I>0 & (xg-seed(1)).^2+(yg-seed(2)).^2+(zg-seed(3)).^2<=radius.^2;
if ~any(region(:)) && I(seed(1),seed(2),seed(3))>0
    region(seed(1),seed(2),seed(3))=true;
end
J=I;
J(~region)=0;
end
