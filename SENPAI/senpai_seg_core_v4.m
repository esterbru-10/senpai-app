function senpai_seg_core_v4(path_in,im_in,varargin)
    ...
    %
    %       senpai_seg_core(path_in,im_in,path_out,sig_G,size_lim,verbmem,paralpool,clusters,backgroundThreshold)
    %     
    %     
    %     Mandatory inputs:
    ...
    if nargin<0
        ...
        % Collect the Z spacing from common OME-TIFF/ImageJ metadata. If it
        % is unavailable or invalid, use isotropic spacing and tell the user.
        xres=1;
        yres=1;
        zres=1;
        zSpacingFound=false;
        if isfield(info1(1),'ImageDescription') && ~isempty(info1(1).ImageDescription)
            strinfo=char(info1(1).ImageDescription);
            numberPattern='([-+]?(?:\d+\.?\d*|\.\d+)(?:[eE][-+]?\d+)?)';
            metadataPatterns={ ...
                ['PhysicalSizeZ\s*=\s*"?\s*' numberPattern], ...
                ['spacing\s*=\s*"?\s*' numberPattern]};

            for patternIndex=1:numel(metadataPatterns)
                zToken=regexp(strinfo,metadataPatterns{patternIndex}, ...
                    'tokens','once','ignorecase');
                if ~isempty(zToken)
                    parsedZres=str2double(zToken{1});
                    if isfinite(parsedZres) && parsedZres>0
                        zres=parsedZres;
                        zSpacingFound=true;
                        break
                    end
        ...
        if ~zSpacingFound
            warning('SENPAI:MissingZSpacing', ...
                ['Z spacing metadata was not found or is invalid. ' ...
                 'Proceeding with isotropic spacing.']);
        end
    ...
    if nargin>3
        ...
        elseif isempty(sig_G) || any(~isfinite(sig_G(:)))
            error('variable sig_G must contain finite values')
    ...
    nextOptional = 7;
    if numel(varargin) >= 7
        optionalArg = varargin{7};
        if isnumeric(optionalArg) && isscalar(optionalArg)
            backgroundThreshold = double(optionalArg);
            nextOptional = 8;
        else
            progressFcn = optionalArg;
            nextOptional = 8;
        end
    end
    if isempty(progressFcn) && numel(varargin) >= nextOptional
        progressFcn = varargin{nextOptional};
        nextOptional = nextOptional + 1;
    end
    if numel(varargin) >= nextOptional
        cancelFcn = varargin{nextOptional};
    end
    reportProgress(0,'Preparing segmentation');
    checkCancel();
    ...
    if isempty(backgroundThreshold) || isnan(backgroundThreshold)
        th_back=0.02*(2^bitl);
    else
        th_back=max(0,double(backgroundThreshold));
    end
    ...
    totalCrops = length(nxiL)*length(nyiL)*length(k_seq);
    processedCrops = 0;
    ...
    for nxin=1:length(nxiL)
        ...
        for nyin=1:length(nyiL)
            ...
            for k=k_seq
                checkCancel();
                reportProgress(0.02+0.68*(processedCrops/max(totalCrops,1)), ...
                    sprintf('Segmentation crop %d/%d',processedCrops+1,totalCrops));
                % define indexes for the crop
                xinit = max(1,k-safe);
                ...
                if isfile([path_out filesep 'sl' mat2str(k) '_' mat2str(nxi) '_' ...
                    ...
                    disp('...already done!')
                    processedCrops = processedCrops + 1;
                    reportProgress(0.02+0.68*(processedCrops/max(totalCrops,1)), ...
                        sprintf('Segmentation crop %d/%d',processedCrops,totalCrops));
                    continue
                end
    ...
    % recompose crops
    reportProgress(0.70,'Recomposing volumes');
    checkCancel();
    ...
    totalTiles = length(1:win:Nz)*length(1:size_lim(1):Nx)*length(1:size_lim(2):Ny);
    tileCount = 0;
    ...
    for k=1:win:Nz
        for x=1:size_lim(1):Nx
            for y=1:size_lim(2):Ny
                ...
                reportProgress(0.70+0.10*(tileCount/max(totalTiles,1)), ...
                    sprintf('Recomposing crop %d/%d',tileCount,totalTiles));
            end
        end
    end
    ...
    for k=1:win:Nz
        for x=1:size_lim(1):Nx
            for y=1:size_lim(2):Ny
                ...
                tileCount = tileCount + 1;
                reportProgress(0.80+0.08*(tileCount/max(totalTiles,1)), ...
                    sprintf('Level 1 classes %d/%d',tileCount,totalTiles));
            end
        end
    end
        ...
        for k=1:win:Nz
            for x=1:size_lim(1):Nx
                for y=1:size_lim(2):Ny
                    ...
                    tileCount = tileCount + 1;
                    reportProgress(0.88+0.05*(tileCount/max(totalTiles,1)), ...
                        sprintf('Level 2 classes %d/%d',tileCount,totalTiles));
                end
            end
        end
    ...
    reportProgress(0.93,'Generating final mask');
    checkCancel();
    if length(sig_G)>1
    ...
    reportProgress(1,'Segmentation complete');
    disp([im_in ' : DONE.']) 
