...
%read from swc
if size(swc_mat,1)<2 || ~any(swc_mat(:,7)>0)
    error('senpai_strahlerord:InsufficientSkeleton', ...
        ['Strahler statistics require a skeleton with at least one branch. ' ...
         'Run Skeletonize / SWC with a neuron connected to the soma mask.'])
end
xn=round(swc_mat(:,3));yn=round(swc_mat(:,4));zn=round(swc_mat(:,5));zn(zn<1)=1;xn(xn<1)=1;
diam=swc_mat(:,6);root_cand=find(swc_mat(:,7)==-1);
if numel(root_cand)~=1
    error('senpai_strahlerord:InvalidRoot', ...
        'The SWC data must contain exactly one root node.')
end
...
ne=swc_mat(:,1);
validEdges=ni>0;
t=graph(ni(validEdges),ne(validEdges),diam(validEdges),length(ni));
